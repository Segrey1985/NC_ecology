...
class RagPrompt(BaseModel):
    rag_prompt: str = Field(
        ...,
        description=(
            "Прямой семантический запрос для векторного поиска. "
            "Формулируй как естественный вопрос или утверждение, которое могло бы быть заголовком нужного текста. "
            "СТРОГО ЗАПРЕЩЕНЫ мета-фразы ('Здесь говорится о...', 'Фрагмент содержит...'). "
            "Пример ПРАВИЛЬНО: 'Какие охраняемые виды растений обнаружены на участке изысканий?' "
            "Пример ПРАВИЛЬНО: 'Письмо уполномоченного органа об отсутствии краснокнижных растений.' "
            "Используй точные термины, ГОСТы и аббревиатуры из запроса."
        ),
    )
...
class RerankPrompt(BaseModel):
    reranker_prompt: str = Field(
        ...,
        description=(
            "Поисковый запрос для cross-encoder.\n"
            "Обязательные правила:\n"
            "1. Сформулирован в виде вопроса пользователя.\n"
            "2. Раскрой смысл: используй предлоги, союзы и вопросительные слова вместо набора сухих ключевых слов.\n"
            "3. Уточни контекст: устрани двусмысленность, явно укажи подразумеваемые объекты.\n"
            "4. Выдели намерение (интент): четко обозначь, что ищется (перечень мероприятий, номер документа, точное число и т.п.)."
        ),
    )
...
def generate_retrieval_prompts_node(
    ...
    system_message = SystemMessage(
        "Ты — AI-инженер, создающий идеальные поисковые запросы для RAG-системы по проектной документации (строительство, экология). "
        "Твоя задача — трансформировать запрос пользователя в список точных, прямых запросов для векторного поиска и реранкинга. "
        "КРИТИЧЕСКОЕ ПРАВИЛО: Никогда не описывай, что должен содержать текст. "
        "Запросы должны звучать так, как будто их вводит в поисковую строку человек или как заголовки целевых документов. "
        "Отвечай на русском языке, используя сухую терминологию отрасли."
    )
